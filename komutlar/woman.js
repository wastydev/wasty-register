...
module.exports.run = async (client, message, args) => {
  ...
  if(!['kayıtçıid'].some(role => message.member.roles.cache.get(role)) && !message.member.hasPermission('ADMINISTRATOR'))
  ...
  const sunucutag = ''   
  const woman = message.guild.roles.cache.find(r => r.id === '')
  const woman2 = message.guild.roles.cache.find(r => r.id === '')
  const unregister = message.guild.roles.cache.find(r => r.id === '')
  const log = message.guild.channels.cache.find(c => c.id === '')
  const chat = message.guild.channels.cache.find(c => c.id === '');
