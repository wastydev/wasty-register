...
module.exports.run = async (client, message, args) => {
  ...
  if(!['kayıtçı'].some(role => message.member.roles.cache.get(role)) && !message.member.hasPermission('ADMINISTRATOR'))
  ...
  const sunucutag = ''   
  const man = message.guild.roles.cache.find(r => r.id === '')
  const man2 = message.guild.roles.cache.find(r => r.id === '')
